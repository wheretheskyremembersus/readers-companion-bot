...
function unlockLayer() {
  ...
  let buttons = layers[currentLayer]
    .map(q =>
      `<button class="btn ${selectedVoice}" onclick="answerCharacter('${q[0].replace(/'/g,"\\'")}', this)">
        ${q[0]}
      </button>`
    )
    .join("");
...
function answerCharacter(questionText, btn) {
  ...
  for (let layer of layers) {
    for (let q of layer) {
      if (q[0] === questionText) {
        answer = q[1][selectedVoice];
      }
    }
  }

  if (answer) {
    chatBox.innerHTML += `<p class="response ${selectedVoice}">${answer}</p>`;
  }
